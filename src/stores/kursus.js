...
export const useKursusStore = defineStore('kursus', {
  ...
  actions: {
    async fetchKursus() {
      const res = await axios.get('http://localhost:3001/kursus')
      this.kursus = res.data
    },
   async addKursus(kursusBaru) {
    try {
      const res = await axios.post('http://localhost:3001/kursus', kursusBaru)
    ...
    async updateKursus(id, data) {
      await axios.patch(`http://localhost:3001/kursus/${id}`, data)
      this.fetchKursus()
    },
    async deleteKursus(id) {
      await axios.delete(`http://localhost:3001/kursus/${id}`)
      this.fetchKursus()
    }
  }
})
